...
class Absensi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['divisi_user'] = $this->Absensi_model->divisi_user();
    ...
    {
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['userdata'] = $this->user->get();
        $data['divisi_user'] = $this->absensi->divisi_user();
        $this->load->view("layout/header", $data);
        $this->load->view("absensi/list_karyawan", $data);
        $this->load->view("layout/footer", $data);
    }

    public function detail_absensi()
    {
        $data_h['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $id_user = @$this->uri->segment(3) ? $this->uri->segment(3) : $this->session->id;
        ...
        $this->load->view("layout/header", $data_h);
    ...
    public function check_absen()
    {
        ...
        $now = date('H:i:s');
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['absen'] = $this->absensi->absen_harian_user($id_user)->num_rows();
        $this->load->view("layout/header", $data);
        $this->load->view("absensi/absen", $data);
        $this->load->view("layout/footer", $data);
    }
    ...
    public function absen()
    {
        ...
        if (@$this->uri->segment(3)) {
            ...
            $absen_harian = $this->absensi->absen_harian_user($id_user);
